...
# Убедимся, что импортируем правильный ResBlock (который поддерживает SE)
...
class MorseRecognizer(nn.Module):
    ...
    Использует ResBlock с опциональным SE.
    """
    def __init__(self, config: Dict):
        ...
        use_se = model_cfg.get("cnn_use_se", True) # Флаг для SE
        ...
        # --- Создание CNN слоев (только ResBlock) ---
        cnn_layers = []
        ...
        for i in range(num_cnn_blocks):
            ...
            kernel = tuple(cnn_kernels[i])
            stride = tuple(cnn_strides[i])

            # Используем только ResBlock
            cnn_layers.append(
        ...
        # Сохраняем ссылку на конфиг для _calculate_rnn_input_size
        self.config = config
    ...
    def _calculate_rnn_input_size(self, C_in: int, F_in: int, device: str) -> int:
        ...
        try:
            ...
            # Используем F_in и фактор сжатия частот
            cnn_output_freq_dim = math.ceil(F_in / max(1, self._freq_reduction_factor))
            rnn_input_size = cnn_output_channels * cnn_output_freq_dim
