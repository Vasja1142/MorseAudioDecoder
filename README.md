...
*(Замените пути на реальные скриншоты из вашего MLflow)*
...
![Пример Спектрограммы]([путь/к/примеру/spectrogram.png])

*(Опционально: можно добавить пример до и после аугментации)*
...
1.  **Клонируйте репозиторий:**
    ...
    cd MorseAudioDecoder # Или как называется папка проекта
    ```
...
*   **Источник данных:** [Укажите источник, если это публичный датасет, или напишите 'Приватный']
...
*(Данные не включены в этот репозиторий из-за их размера / приватности)*.
...
*   **Основная Метрика:** Расстояние Левенштейна (чем меньше, тем лучше) между предсказанным и реальным текстом.
*   **Лучшая Одиночная Модель (Валидация):** Levenshtein = `[Укажите ваш лучший результат на val_split_df]`
*   **Финальный Ансамбль (Валидация):** Levenshtein = `[Укажите результат ансамбля на val_split_df, если измеряли]`
*   **Финальный Ансамбль (Тест/Kaggle):** [Укажите результат на скрытом тесте или Kaggle LB, если применимо]
